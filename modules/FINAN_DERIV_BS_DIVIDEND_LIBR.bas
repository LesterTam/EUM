...
'************************************************************************************
'************************************************************************************
'FUNCTION      : OPTION_DIVIDEND_ADJ_STOCK_PRICE_FUNC
...
Function OPTION_DIVIDEND_ADJ_STOCK_PRICE_FUNC(ByVal S_VAL As Double, _
...
OPTION_DIVIDEND_ADJ_STOCK_PRICE_FUNC = S_VAL * Exp(-DY_VAL * T_VAL)
'Also uses it for Theta Calc =S*EXP(-Div_Yield*Theta_Time)
Exit Function
ERROR_LABEL:
OPTION_DIVIDEND_ADJ_STOCK_PRICE_FUNC = Err.number
End Function
...
'************************************************************************************
'************************************************************************************
'FUNCTION      : OPTION_DIVIDEND_GROWTH_FUNC
'DESCRIPTION   : DIVD GROWTH-YIELD ESTIMATION
'LIBRARY       : DERIV_BS
'GROUP         : DIVIDEND
'ID            : 006
'LAST UPDATE   : 27/02/2008
...
Function OPTION_DIVIDEND_GROWTH_FUNC(ByVal SETTLEMENT As Date, _
ByVal START_DATE As Date, _
ByVal END_DATE As Date, _
...
Next i
'SROW = MATRIX_FIND_ELEMENT_FUNC(DATES_VECTOR, START_DATE, 1, 1, 1)
'NROWS = MATRIX_FIND_ELEMENT_FUNC(DATES_VECTOR, END_DATE, 1, 1, 1)
...
'---------------------------------------------------------------------------------------------------------
    ...
    Next i
    OPTION_DIVIDEND_GROWTH_FUNC = SPOT - TEMP_SUM 'Adjusted Spot Price for
    'the European option Model (stock with cash dividends)
'---------------------------------------------------------------------------------------------------------
Case 1 'THEORETICAL PRICE (DDM)
'---------------------------------------------------------------------------------------------------------
    TEMP_SUM = 0
    For i = 1 To NSIZE
        TEMP_SUM = TEMP_SUM + TEMP_MATRIX(i, 2) / (1 + COST_VAL) ^ TEMP_MATRIX(i, 1)
    Next i
    OPTION_DIVIDEND_GROWTH_FUNC = TEMP_SUM
'---------------------------------------------------------------------------------------------------------
Case 2
'---------------------------------------------------------------------------------------------------------
    OPTION_DIVIDEND_GROWTH_FUNC = CAGR_VAL 'Compounded Annual Growth Rate
'---------------------------------------------------------------------------------------------------------
Case Else
'---------------------------------------------------------------------------------------------------------
    OPTION_DIVIDEND_GROWTH_FUNC = COST_VAL 'Cost Equity Using Gordon Model
'---------------------------------------------------------------------------------------------------------
End Select
'---------------------------------------------------------------------------------------------------------
...
Exit Function
ERROR_LABEL:
OPTION_DIVIDEND_GROWTH_FUNC = Err.number
End Function
