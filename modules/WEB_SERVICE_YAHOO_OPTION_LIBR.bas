...
Function YAHOO_OPTIONS_QUOTES_FUNC(ByVal TICKERS_RNG As Variant, _
Optional ByVal EXPIRY_STR As String = "", _
Optional ByVal STRIKE_STR As String = "")
...
'-----------------------------------------------------------------------------------------------------------------------------------------
If IsArray(TICKERS_RNG) = True Then 'TICKERS Format --> CSCO110416C00014000
'-----------------------------------------------------------------------------------------------------------------------------------------
    URL_STR = "http://finance.yahoo.com/q?s="
    ...
    HEADINGS_STR = "Ticker,Description,Last Trade,Trade Time,Change,Prev Close,Open,Bid,Ask,Day's Range,Contract Range,Volume,Open Interest,Strike,Expire Date,"
    j = Len(HEADINGS_STR)
    NCOLUMNS = 0
    For i = 1 To j
    ...
    Next k
    For ii = 1 To NROWS
        jj = 1: TEMP_MATRIX(ii, jj) = TICKERS_VECTOR(ii, 1)
        DATA_STR = SAVE_WEB_DATA_PAGE_FUNC(URL_STR & TEMP_MATRIX(ii, 1), 0, True, 0, False, 1, 32767, 0)
        i = 1: LOOK_STR = "Summary for "
        jj = 2: GoSub LOOK_LINE
        j = InStr(i, DATA_STR, "-")
        TEMP_MATRIX(ii, jj) = Mid(DATA_STR, i, j - i)
        For jj = 3 To NCOLUMNS
            TEMP_MATRIX(ii, jj) = RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR & TEMP_MATRIX(ii, 1), 1, TEMP_MATRIX(0, jj) & ":", , , , 0, "</table", , "N/A")
        Next jj
1983:
    Next ii
'-----------------------------------------------------------------------------------------------------------------------------------------
Else 'TICKER Format --> CSCO
'-----------------------------------------------------------------------------------------------------------------------------------------
    '-------------------------------------------------------------------------------------------------------------------------------------
    If EXPIRY_STR = "" And STRIKE_STR = "" Then
    '-------------------------------------------------------------------------------------------------------------------------------------
        TEMP_VECTOR = YAHOO_OPTIONS_EXPIRY_STRIKE_FUNC(TICKERS_RNG, 0)
        If IsArray(TEMP_VECTOR) = False Then: GoTo ERROR_LABEL
        h = UBound(TEMP_VECTOR, 1)
        NROWS = 0
        For i = 1 To h
            NROWS = NROWS + TEMP_VECTOR(i, 2)
        Next i
        ...
        For k = 1 To h
            l = TEMP_VECTOR(k, 2)
            URL_STR = TEMP_VECTOR(k, 1)
            For i = 1 To l
                DATE_STR = TEMP_VECTOR(k, 1): GoSub EXPIRY_LINE: TEMP_MATRIX(m, 1) = SPOT_VAL: TEMP_MATRIX(m, 2) = DATE_VAL 'DATE_STR
                For j = 1 To NCOLUMNS - 2
                    TEMP_MATRIX(m, 2 + j) = RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR, j, "Options Expiring", "Symbol", , , i, "</table", , "N/A")
                Next j
                m = m + 1
            Next i
        Next k
        TEMP_MATRIX = MATRIX_QUICK_SORT_FUNC(TEMP_MATRIX, 2, 0)
    '-------------------------------------------------------------------------------------------------------------------------------------
    ElseIf EXPIRY_STR <> "" Then
    '-------------------------------------------------------------------------------------------------------------------------------------
        TEMP_VECTOR = YAHOO_OPTIONS_EXPIRY_STRIKE_FUNC(TICKERS_RNG, 0)
        If IsArray(TEMP_VECTOR) = False Then: GoTo ERROR_LABEL
        h = UBound(TEMP_VECTOR, 1)
        NROWS = 0
        For i = 1 To h
            If TEMP_VECTOR(i, 3) = EXPIRY_STR Then
                l = i
                NROWS = TEMP_VECTOR(l, 2)
                Exit For
            End If
        Next i
        If NROWS = 0 Then: GoTo ERROR_LABEL
        GoSub HEADINGS_LINE
        URL_STR = TEMP_VECTOR(l, 1)
        For i = 1 To NROWS
            DATE_STR = EXPIRY_STR: GoSub EXPIRY_LINE: TEMP_MATRIX(i, 1) = SPOT_VAL: TEMP_MATRIX(i, 2) = DATE_VAL 'DATE_STR
            For j = 1 To NCOLUMNS - 2
                TEMP_MATRIX(i, 2 + j) = RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR, j, "Options Expiring", "Symbol", , , i, "</table", , "N/A")
            Next j
        Next i
    '-------------------------------------------------------------------------------------------------------------------------------------
    ElseIf STRIKE_STR <> "" Then
    '-------------------------------------------------------------------------------------------------------------------------------------
        URL_STR = "http://finance.yahoo.com/q/os?s=" & TICKERS_RNG & "&k=" & STRIKE_STR
        NROWS = 0: DATE_STR = ""
        Do
            NROWS = NROWS + 1
            DATE_STR = Trim(RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR, 8, "Strike Price at", "Symbol", , , NROWS, "</table", , ""))
        Loop Until DATE_STR = ""
        NROWS = NROWS - 1
        GoSub HEADINGS_LINE
        For i = 1 To NROWS
            DATE_STR = CStr(RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR, 8, "Strike Price at", "Symbol", , , i, "</table", , "N/A"))
            DATE_STR = "20" & Mid(DATE_STR, 1, 2) & "-" & Mid(DATE_STR, 4, 2)
            GoSub EXPIRY_LINE
            TEMP_MATRIX(i, 1) = SPOT_VAL
            TEMP_MATRIX(i, 2) = DATE_VAL
            For j = 1 To NCOLUMNS - 2
                If j <> 8 Then
                    TEMP_MATRIX(i, 2 + j) = RETRIEVE_WEB_DATA_CELL_FUNC(URL_STR, j, "Strike Price at", "Symbol", , , i, "</table", , "N/A")
                Else
                    TEMP_MATRIX(i, 2 + j) = CDbl(STRIKE_STR)
                End If
            Next j
        Next i
        TEMP_MATRIX = MATRIX_QUICK_SORT_FUNC(TEMP_MATRIX, 2, 0)
...
YAHOO_OPTIONS_QUOTES_FUNC = TEMP_MATRIX
...
HEADINGS_LINE:
'----------------------------------------------------------------------------------------
    HEADINGS_STR = "Stock Price,Expiring,Call Symbol,Call Last,Call Change,Call Bid,Call Ask,Call Volume,Call Open Int,Strike Price,Put Symbol,Put Last,Put Change,Put Bid,Put Ask,Put Volume,Put Open Int,"
    j = Len(HEADINGS_STR)
    NCOLUMNS = 0
    For i = 1 To j
        If Mid(HEADINGS_STR, i, 1) = "," Then: NCOLUMNS = NCOLUMNS + 1
    ...
    For k = 1 To NCOLUMNS
        ...
        i = j + 1
    Next k
    SPOT_VAL = MATRIX_YAHOO_QUOTES_FUNC(TICKERS_RNG, "l1", "", , False, "+", 0, 0)(1, 1)
'----------------------------------------------------------------------------------------
Return
'----------------------------------------------------------------------------------------
EXPIRY_LINE:
'----------------------------------------------------------------------------------------
    ii = InStr(1, DATE_STR, "m=")
    If ii = 0 Then: GoTo 1984
    ii = ii + 2
    jj = Len(DATE_STR)
    DATE_STR = Mid(DATE_STR, ii, jj - ii + 1)
1984:
    ii = 1: jj = InStr(ii, DATE_STR, "-")
    If jj = 0 Then: GoTo 1985
    kk = CLng(Mid(DATE_STR, ii, jj - ii))
    
    ii = jj + 1: jj = Len(DATE_STR)
    If jj = 0 Then: GoTo 1985
    ll = CLng(Mid(DATE_STR, ii, jj - ii + 1))
    DATE_VAL = GET_OPTION_EXPIRY_FUNC(kk, ll, "M")
1985:
'----------------------------------------------------------------------------------------
Return
'----------------------------------------------------------------------------------------
ERROR_LABEL:
YAHOO_OPTIONS_QUOTES_FUNC = Err.number
End Function
