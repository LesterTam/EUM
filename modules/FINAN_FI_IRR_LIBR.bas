...
Dim X_VAL As Double
Dim Y_VAL As Double 'f(x)
Dim YD_VAL As Double 'f'(x)
...
TEMP_MATRIX(0, 1) = "x": TEMP_MATRIX(0, 2) = "f(x)": TEMP_MATRIX(0, 3) = "f'(x)"

i = 1
TEMP_MATRIX(i, 1) = XGUESS_VAL
X_VAL = TEMP_MATRIX(i, 1)
GoSub CALC_LINE
TEMP_MATRIX(i, 2) = Y_VAL
TEMP_MATRIX(i, 3) = YD_VAL

For i = 2 To NBINS 'Solve f(x) = 0
    TEMP_MATRIX(i, 1) = TEMP_MATRIX(i - 1, 1) - TEMP_MATRIX(i - 1, 2) / TEMP_MATRIX(i - 1, 3) 'Newton
    X_VAL = TEMP_MATRIX(i, 1)
    GoSub CALC_LINE
    TEMP_MATRIX(i, 2) = Y_VAL
    TEMP_MATRIX(i, 3) = YD_VAL
...
Select Case OUTPUT
Case 0
    IRR_CHARACTERISTICS_SOLVER_FUNC = TEMP_MATRIX
Case Else 'IRR
    IRR_CHARACTERISTICS_SOLVER_FUNC = TEMP_MATRIX(NBINS, 1)
End Select
Exit Function
'--------------------------------------------------------------------------------------------------------------------------
CALC_LINE:
'--------------------------------------------------------------------------------------------------------------------------
    Y_VAL = Excel.Application.Evaluate(Replace(YFORMULA_STR, "x", X_VAL))
    'YFORMULA_STR = 10 * (1 + X_VAL) ^ 5 - 5 * (1 + X_VAL) ^ 4 - 5 * (1 + X_VAL) ^ 3 - 5 * (1 + X_VAL) ^ 2 - 5 * (1 + X_VAL) - 5
    YD_VAL = Excel.Application.Evaluate(Replace(YDFORMULA_STR, "x", X_VAL))
    'YDFORMULA_STR = 50 * (1 + X_VAL) ^ 4 - 20 * (1 + X_VAL) ^ 3 - 15 * (1 + X_VAL) ^ 2 - 10 * (1 + X_VAL) - 5
'--------------------------------------------------------------------------------------------------------------------------
Return
'--------------------------------------------------------------------------------------------------------------------------
ERROR_LABEL:
IRR_CHARACTERISTICS_SOLVER_FUNC = Err.number
End Function
