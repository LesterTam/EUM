...
Private Const PUB_ADVFN_SERVER_STR As String = "www" 'ca
...
Optional ByVal VERSION As Long = 0, _
Optional ByVal SERVER_STR As String = "ca")
...
If SERVER_STR = "" Then: SERVER_STR = PUB_ADVFN_SERVER_STR
...
'-----------------------------------------------------------------------------
    ...
    If InStr(1, TICKER_STR, ":") > 0 Then: TICKER_STR = ESCAPE_URL_FUNC(TICKER_STR)
    SRC_URL_STR = "http://" & SERVER_STR & PUB_ADVFN_URL_STR & "&symbol=" & TICKER_STR
...
Optional ByVal CURRENCY_VAL As Long = 1, _
Optional ByVal SERVER_STR As String = "ca")
...
'US Dollars --> AMOUNTS_VAL= 1
'British Pounds --> AMOUNTS_VAL= 2
'Canadian Dollars --> AMOUNTS_VAL= 3
'Euros --> AMOUNTS_VAL= 4
'Japanese Yen --> AMOUNTS_VAL= 5
'Swedish Krona --> AMOUNTS_VAL= 6
...
KEY_STR = TICKER_STR & "|" & QUATERLY_FLAG & "|" & NO_PERIODS & "|" & CURRENCY_VAL
'-----------------------------------------------------------------------
...
'-----------------------------------------------------------------------
    ...
    End If
    For i = 1 To UBound(ELEMENT_VECTOR, 1)
...
If InStr(1, TICKER_STR, ":") > 0 Then: TICKER_STR = ESCAPE_URL_FUNC(TICKER_STR)
SRC_URL_STR = "http://" & SERVER_STR & PUB_ADVFN_URL_STR & "&btn="

If QUATERLY_FLAG = False Then
    CHR_STR = "&"
    LOOK_STR = "name='start_date'"
    REPORT_STR = "annual_reports"
Else
    CHR_STR = "&i"
    LOOK_STR = "name='istart_date'"
    REPORT_STR = "quarterly_reports"
End If

SRC_URL_STR = SRC_URL_STR & _
    REPORT_STR & "&symbol=" & TICKER_STR & _
    "&" & REPORT_STR & "=" & CStr(CURRENCY_VAL)
...
SRC_URL_STR = SRC_URL_STR & CHR_STR & "start_date=" & CStr(NO_PERIODS)
kk = PARSE_ADVFN_STATEMENTS_FUNC(SRC_URL_STR, TEMP_MATRIX, LOOK_STR)
If kk = 0 Or IsArray(TEMP_MATRIX) = False Then: GoTo ERROR_LABEL
kk = kk + 1 'Last period, plus one index
...
Next i
'RETRIEVE_WEB_DATA_CELL_FUNC(SRC_URL_STR, jj, "Indicators",
'">" & ELEMENT_STR, , , , "</TABLE", , "--")
    
If kk <= NO_PERIODS Then
    k = kk + 1
    j = 0
Else
    k = NO_PERIODS + 1
...
Do
    SRC_URL_STR = SRC_URL_STR & CHR_STR & "start_date=" & CStr(j)
    Call PARSE_ADVFN_STATEMENTS_FUNC(SRC_URL_STR, TEMP_MATRIX, "")
    If IsArray(TEMP_MATRIX) = False Then: Exit Do
    j = j + (UBound(TEMP_MATRIX, 2) - 1)
        
    For jj = 2 To UBound(TEMP_MATRIX, 2)
...
1982:
'---------------------------------------------------------------------------------------
If IsArray(ELEMENT_VECTOR) = True Then
'---------------------------------------------------------------------------------------
    ...
    For i = 1 To NROWS 'Indexing Entries
        TEMP_STR = LCase(Trim(CStr(DATA_MATRIX(i, 1)))) '
        If ELEMENTS_HASH_OBJ.Exists(TEMP_STR) = False Then
            Call ELEMENTS_HASH_OBJ.Add(TEMP_STR, CStr(i))
        Else
            'Check for duplicates such as depreciation, amortization,
            'deferred-taxes
    ...
    For i = 1 To NROWS
        'TEMP_STR = LCase(Trim(CStr(ELEMENT_VECTOR(i, 1))))
        ...
        For j = 1 To NCOLUMNS
            TEMP_MATRIX(i, j) = DATA_MATRIX(k, j + 1)
        Next j
1983:
    Next i
    Erase DATA_MATRIX
    Set ELEMENTS_HASH_OBJ = Nothing
...
Function ADVFN_STATEMENT_ELEMENTS_FUNC(Optional ByVal VERSION As Long = 0, _
Optional ByVal OUTPUT As Long = 2, _
Optional ByVal QUATERLY_FLAG As Boolean = True)
...
'--------------------------------------------------------------------------
    If OUTPUT = 0 Then
        ReDim TEMP_GROUP(1 To 18, 1 To 2)
        k = 0
        For l = 1 To 18
...
'--------------------------------------------------------------------------
    If OUTPUT = 0 Then
        ReDim TEMP_GROUP(1 To 9, 1 To 2)
        k = 0
        For l = 1 To 9
            TEMP_GROUP(l, 1) = ADVFN_STATEMENT_ELEMENTS_FUNC(1, l, QUATERLY_FLAG)
            TEMP_GROUP(l, 2) = UBound(TEMP_GROUP(l, 1))
            k = k + TEMP_GROUP(l, 2)
        Next l
        ReDim TEMP_VECTOR(1 To k, 1 To 1)
